...
use tjuae_config::compat::{OpenAiApiMode, ProviderCompat};
use tjuae_providers::LlmProvider;
use tjuae_providers::openai::OpenAIProvider;
use tjuae_types::llm::{LlmEvent, LlmRequest};
use tjuae_types::message::{ContentBlock, Message, Role, StopReason};
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        tjuae_providers::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        tjuae_providers::ProviderError::RateLimited { retry_after_ms, body } => {
