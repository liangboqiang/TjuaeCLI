...
mod tests {
    ...
    fn test_api_key_from_cli_arg() {
        ...
        let result = resolve_api_key(Some("cli-key"), Some("config-key"), ProviderType::Anthropic, None).unwrap();
    ...
    fn test_api_key_from_config() {
        ...
        let result = resolve_api_key(None, Some("config-key"), ProviderType::Anthropic, None).unwrap();
    ...
    fn test_api_key_missing_returns_error() {
        ...
        let result = resolve_api_key(None, None, ProviderType::Anthropic, None);
    ...
    fn test_api_key_bedrock_returns_empty_without_key() {
        ...
        let result = resolve_api_key(None, None, ProviderType::Bedrock, None).unwrap();
        assert_eq!(result, "");
    }
    ...
    fn test_api_key_vertex_returns_empty_without_key() {
        ...
        let result = resolve_api_key(None, None, ProviderType::Vertex, None).unwrap();
        assert_eq!(result, "");
    }
...
"#;
    ...
    fn test_resolve_with_project_dir_loads_project_config() {
        ...
        let project_toml = tmp.path().join(".tjuae.toml");
        std::fs::write(
            &project_toml,
            r#"
[default]
