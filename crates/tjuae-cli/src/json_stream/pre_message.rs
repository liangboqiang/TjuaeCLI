    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "tjuae_mcp", %name, %transport, ?command, "AddMcpServer received");
                ...
                tracing::info!(target: "tjuae_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        tracing::info!(target: "tjuae_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "tjuae_mcp", %name, error = %e, "mcp server connection failed");
