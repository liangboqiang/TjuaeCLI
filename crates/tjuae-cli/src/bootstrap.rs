//! Shared initialization steps for TjuaeCLI entry points.
//!
...
//!
//! Note: `tjuae_agent` also exposes a module named `bootstrap`
//! (`tjuae_agent::bootstrap::AgentBootstrap`). Call sites should keep using
//! that fully-qualified path for the engine builder itself; this module is
//! `crate::bootstrap` and only wraps it.

use std::sync::Arc;

use tjuae_agent::bootstrap::{AgentBootstrap, BootstrapResult};
use tjuae_agent::output::OutputSink;
use tjuae_agent::session::{Session, SessionManager};
use tjuae_compact::CompactLevel;
use tjuae_config::config::{CliArgs, Config};
use tjuae_config::logging::{LoggingGuard, create_file_layer};
