...
const MEMORY_DIR_ENV: &str = "TJUAE_MEMORY_DIR";
...
///
/// Resolution order:
///   1. `TJUAE_MEMORY_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `tjuae-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    tjuae_config::config::app_config_dir()
}
