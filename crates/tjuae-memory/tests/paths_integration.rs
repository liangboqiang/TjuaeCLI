...
fn tc_2_1_default_base_dir_uses_platform_config() {
    ...
    // Should use the Tjuae runtime directory (not a provider-specific path).
    assert!(
        base.to_string_lossy().contains("tjuae"),
        "base dir should use the tjuae runtime directory: {base:?}"
    );
