...
mod tests_set_config {
    ...
    use tjuae_config::compat::ReasoningCompat;
    use tjuae_providers::error::ProviderError;
    use tjuae_providers::provider::LlmProvider;
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            compact_config: tjuae_config::compact::CompactConfig::default(),
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(tjuae_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(tjuae_types::llm::ThinkingConfig::Enabled { budget_tokens: 8000 });
        let changes = engine.apply_config_update(None, None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(tjuae_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_thinking_budget_only_updates_existing_enabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(tjuae_types::llm::ThinkingConfig::Enabled { budget_tokens: 5000 });
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(tjuae_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(tjuae_types::llm::ThinkingConfig::Disabled) => {}
...
mod tests_phase6 {
    ...
    use tjuae_tools::registry::ToolRegistry;
    use tjuae_types::llm::{LlmEvent, LlmRequest};
    use tjuae_types::skill_types::{ContextModifier, EffortLevel};
...
mod tests_compact {
    ...
    use tjuae_tools::registry::ToolRegistry;
    use tjuae_types::llm::{LlmEvent, LlmRequest};
    use tjuae_types::message::{ContentBlock, ImageInputCapability, ImageUrl, Message, Role, StopReason, TokenUsage};
    use tokio::sync::mpsc;
...
mod tests_handle_command {
    ...
    #[tokio::test]
    async fn run_treats_tjuae_files_marker_as_plain_text() {
        let mut engine = make_engine_with_provider(Arc::new(SingleResponseProvider));
        let input = "discuss [[TJUAE_FILES]] and file.txt";
...
mod tests_loop_helpers {
    use serde_json::json;
    use tjuae_types::message::{ContentBlock, StopReason, TokenUsage};
...
mod tests_tool_policy_enforcement {
    ...
    use tjuae_tools::registry::ToolRegistry;
    use tjuae_types::llm::{LlmEvent, LlmRequest};
    use tjuae_types::message::{ContentBlock, ImageInputCapability};
    use tjuae_types::tool::ToolResult;
