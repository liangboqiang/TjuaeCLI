...
use tjuae_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
...
use tjuae_types::skill_types::ContextModifier;
...
pub async fn execute_tool_calls(
    ...
    compaction_level: tjuae_compact::CompactLevel,
    toon_enabled: bool,
...
async fn execute_single(
    ...
    let (result, modifier, follow_up_blocks) = match registry.get(name) {
        Some(tool) => {
            ...
            let content = tjuae_compact::compact_output(&content, compaction_level);
            let content = if toon_enabled {
                tjuae_compact::compact_output_toon(&content)
            } else {
                content
            };
            (
