...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> tjuae_types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return tjuae_types::tool::ToolResult {
        ...
        fn category(&self) -> tjuae_protocol::events::ToolCategory {
            tjuae_protocol::events::ToolCategory::Exec
        }
    }
    ...
    async fn execute_single_deferred_tool_error_missing_required_appends_hint() {
        ...
        let (result, _, follow_up_blocks) =
            execute_single(&registry, &call, None, tjuae_compact::CompactLevel::Off, false).await;
