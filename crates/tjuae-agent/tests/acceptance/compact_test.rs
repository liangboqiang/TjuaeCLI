...
use serde_json::json;
use tjuae_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use tjuae_agent::compact::emergency::is_at_emergency_limit;
use tjuae_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
...
async fn autocompact_triggers_llm_summary() {
    ...
    let provider = tjuae_providers::create_provider(&config);
