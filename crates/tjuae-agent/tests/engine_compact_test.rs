...
use tjuae_agent::error::AgentError;
...
async fn tc_2_6_02_micro_before_auto_execution_order() {
    ...
    let captured: Arc<Mutex<Option<Vec<tjuae_types::message::Message>>>> = Arc::new(Mutex::new(None));
    let capture_ref = captured.clone();

    struct OrderProvider {
        regular_count: Mutex<usize>,
        captured: Arc<Mutex<Option<Vec<tjuae_types::message::Message>>>>,
    }
    ...
    let cleared_count = msgs
        ...
        .filter(|b| {
            matches!(
                b,
                tjuae_types::message::ContentBlock::ToolResult { content, .. }
                    if content == tjuae_agent::compact::micro::CLEARED_TOOL_RESULT
            )
        })
        .count();
