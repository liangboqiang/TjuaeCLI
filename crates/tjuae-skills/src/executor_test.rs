...
mod tests {
    ...
    async fn test_prepare_inline_substitutes_skill_dir() {
        let skill = make_skill("Dir: ${TJUAE_SKILL_DIR}", Some("/skills/mine"));
        let result = prepare_inline_content(&skill, None, None, Path::new("/tmp"))
            .await
            .unwrap();
    ...
    async fn test_prepare_inline_substitutes_session_id() {
        let skill = make_skill("Session: ${TJUAE_SESSION_ID}", None);
...
mod supplemental_tests {
    ...
    async fn tc_10_x_session_id_substituted() {
        let skill = make_skill_full("s", "${TJUAE_SESSION_ID}", None, vec![], ExecutionContext::Inline);
...
mod phase7_tests {
    ...
    use tjuae_types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
