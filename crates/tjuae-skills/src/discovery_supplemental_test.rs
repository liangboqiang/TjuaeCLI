...
mod discovery_supplemental_tests {
    ...
    /// Create a `.tjuae/skills/` directory inside `parent`.
    fn create_skill_dir(parent: &Path) -> PathBuf {
        let dir = parent.join(".tjuae").join("skills");
    ...
    // TC-22: discovers `.tjuae/skills/` inside a direct subdirectory of cwd.
    #[tokio::test]
    async fn tc22_discover_dirs_finds_tjuae_cli_skills_in_subdir() {
        let tmp = TempDir::new().unwrap();
        let cwd = tmp.path().to_str().unwrap().to_string();

        // Create /tmp/proj/module/.tjuae/skills/
        ...
        assert!(found[0].ends_with(".tjuae/skills"));
    }
    ...
    // TC-23: `.tjuae/skills/` at cwd level is not returned (loaded at startup).
    #[tokio::test]
    ...
    // TC-25: directories without `.tjuae/skills/` are still recorded to avoid
    // repeated stat calls.
    #[tokio::test]
