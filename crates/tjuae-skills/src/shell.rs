...
use tjuae_process::{CommandRunner, DEFAULT_TIMEOUT};
...
async fn execute_command(command: &str, cwd: &Path) -> Result<String, ShellExecutionError> {
    let shell = tjuae_config::shell::default_shell();
    let mut command_builder = tjuae_config::shell::shell_command_builder(&shell, command, false);
