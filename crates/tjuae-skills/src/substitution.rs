...
/// 5. Skill directory: `${TJUAE_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${TJUAE_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    if let Some(root) = skill_root {
        result = result.replace("${TJUAE_SKILL_DIR}", root);
    }

    // 6. ${TJUAE_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${TJUAE_SESSION_ID}", sid);
    }
