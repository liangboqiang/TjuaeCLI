...
// AC-5: $ARGUMENTS, $0, ${TJUAE_SKILL_DIR} are correctly substituted
// ---------------------------------------------------------------------------
...
async fn tc_e2e_2_inline_variable_substitution() {
    ...
    assert!(
        ...
        "${{TJUAE_SKILL_DIR}} should be replaced with skill root, got: {result}"
    );
    ...
    assert!(
        !result.contains("${TJUAE_SKILL_DIR}"),
        "${{TJUAE_SKILL_DIR}} literal should not remain, got: {result}"
    );
}
