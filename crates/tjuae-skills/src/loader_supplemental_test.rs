...
async fn tc_11_1_bare_mode_only_loads_add_dirs() {
    ...
    // Put a skill in add_dir's .tjuae/skills/
    let add_skills_dir = add_tmp.path().join(".tjuae").join("skills");
...
async fn tc_11_5_empty_scenario_returns_empty_vec() {
    ...
    let tmp = TempDir::new().unwrap();
    // tmp exists but has no .tjuae/skills
...
// matching TS skillRoot used for ${TJUAE_SKILL_DIR} substitution.
// -----------------------------------------------------------------------
