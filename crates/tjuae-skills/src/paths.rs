...
use tjuae_config::config::app_config_dir;

// ---------------------------------------------------------------------------
// User-level directories (<config_dir>/tjuae/)
// ---------------------------------------------------------------------------

/// Return the user-level skills directory: `<config_dir>/tjuae/skills/`
///
...
/// Find all project-level `.tjuae/skills/` directories by walking up from
...
/// `.tjuae/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".tjuae").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
