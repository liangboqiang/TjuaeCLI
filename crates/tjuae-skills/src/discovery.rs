...
/// Manages runtime discovery of `.tjuae/skills/` directories found in
...
impl RuntimeDiscovery {
    ...
    /// Discover `.tjuae/skills/` directories by walking up from each file path to `cwd`.
    ///
    ...
    pub async fn discover_dirs_for_paths(&mut self, file_paths: &[&str], cwd: &str) -> Vec<PathBuf> {
        ...
        for &file_path in file_paths {
            ...
            loop {
                ...
                let skill_dir = current.join(".tjuae").join("skills");
    ...
    /// already known not to contain a `.tjuae/skills/` subdirectory.
